...
class SMTP{
	 ...
	 */
	public function send($to, $from = "", $subject = "", $body = "", $additional_headers = "", $mailtype = "HTML", $cc = "", $bcc = "", $message_id = "",$attachments=[]) {
		...
		if($attachments){
			...
			foreach($attachments as $k => $attach){
				...
				$headers[] = "Content-Disposition: attachment; filename=\"=?".$this->charset."?B?".base64_encode(basename($filePath)).'?="'.self::CRLF;
			...
			$header .= str_replace(self::CRLF.'.', self::CRLF.'..', trim(implode(self::CRLF, $headers)));
		}else{
			if($mailtype == "HTML"){
				$body = preg_replace("/(^|(\r\n))(\\.)/i", "\\1.\\3", $body);
	 ...
	 * @return string
	 */
	public function mimeHeader(Mail $mail, $body, $toAddress){
